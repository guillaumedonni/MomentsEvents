...
export default function CategoryForm(id) {
    ...
    const renderPhotos = (source) => {
        return source.map(photo=>{
            // console.log(photo)
            return <img src={photo.split('|').at(0)} alt="" key={photo} style={{ width:'20%',height:'180px',margin:'0 5px' }} />
        })
    }
    ...
    // IF IDPARAM THEN A CATEGORIE IS PASSED TO THIS COMPONENT 
    // ELSE THERE IS AN INSERT OF A NEW CATEGORY HAPPENING
    if(idParam) {
        useEffect(()=>{
    ...
    const getCategories = async () => {
        setLoading(true)
        await axiosClient.get(`/admin/categories/${idParam}`)
        .then(({data})=>{
            // console.log('data => ',data)
            const {nom,score,image,created_at,updated_at} = data.category
            // console.log('id=',id)
    ...
    const handleModify = async (e) => {
        ...
        await axiosClient.post(`/admin/updateCategorie/${idParam}`, formData)
        .then((data)=>{
            // console.log(data)
